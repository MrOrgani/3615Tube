...
const resolvers: ResolverMap = {
    Mutation: {
        removeCommentary:  createMiddleware(verifyAndSetSession,
            async (_: any, {id, imdbId}: GQL.IRemoveCommentaryOnMutationArguments, { session }: any ) => {
                const DeleteCommentary = await Commentary.findOne({where: {author: session.user.id, id: id,film_id: imdbId}})
